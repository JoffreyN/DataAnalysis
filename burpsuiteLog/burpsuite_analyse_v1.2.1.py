...
def getallList(allLines):
    ...
    count=len(allList)
    #print(allList)
    for i in range(count):#请求中所有字段及值加起来的长度小于100会被丢弃
...
def saveExcel(allDic):
    #row0是表头，如果发现某个自定义字段出现的次数较多，可将该字段添加进row0内，放在'otherKeys'之前
    row0=['GET','POST','PostData','Host','Accept', 'Accept-Charset', 'Accept-Encoding',
         ...
         'User-Agent', 'Via', 'Warning','HEAD','X-Requested-With','Upgrade-Insecure-Requests','Origin','otherKeys']
    excel=Workbook()
    sheet=excel.active
    z=0
    print('\n',end='')
